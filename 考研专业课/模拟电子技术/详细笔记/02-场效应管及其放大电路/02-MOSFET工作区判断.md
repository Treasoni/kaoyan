...
#### 5.1.4 手写补充：零偏耗尽型 NMOS 共源放大

![[考研专业课/模拟电子技术/附件/02-场效应管及其放大电路/手写-耗尽型NMOS零偏共源-直流交流与动态指标-2026-08-06.png|620]]

![[考研专业课/模拟电子技术/附件/02-场效应管及其放大电路/图-耗尽型NMOS零偏共源-直流交流对照.png|780]]

> [!note] 图的作用：同一电路必须分成直流、交流两次画
> 直流图中栅极只经 $R_G$ 接地，得到 $V_{GSQ}=0$；交流图中 $V_{DD}$ 是交流地，$R_D$ 与耦合到输出端的 $R_L$ 并联。图中器件本身是耗尽型 NMOS，和 JFET 的 PN 结栅不是一回事。

##### 直流：$V_{GSQ}=0$ 不等于“无条件地 $I_{DQ}=I_{DSS}$”

由于理想 MOS 栅极电流近似为零，$R_G$ 上没有直流压降；又因为源极直接接地：

$$
V_G=0,\qquad V_S=0,\qquad V_{GSQ}=0
$$

若 Q 点处在恒流区，转移特性在 $V_{GS}=0$ 的读数就是 $I_{DSS}$，所以：

$$
I_{DQ}=I_{DSS},\qquad V_{DSQ}=V_{DD}-I_{DSS}R_D
$$

但这个结论要补上恒流区校验：

$$
V_{DSQ}\geq V_{GSQ}-V_{GS(\mathrm{off})}
=-V_{GS(\mathrm{off})}
$$

若校验不成立，不能仍把 $I_{DQ}$ 取作 $I_{DSS}$；应在输出特性上画直流负载线

$$
i_D=\frac{V_{DD}-v_{DS}}{R_D}
$$

并与 $v_{GS}=0$ 的输出特性曲线取交点 Q，再读出 $I_{DQ}$、$V_{DSQ}$。

##### 交流：共源反相，动态指标由 $g_m$ 和漏极等效负载决定

中频时耦合电容近似短路、$V_{DD}$ 为交流地。若忽略 $r_{ds}$，令
...
$$
\boxed{A_u=\frac{v_o}{v_i}=-g_m(R_D\parallel R_L)}
$$

手写中写成 $U_i/U_o$ 是分子、分母颠倒；负号说明共源放大器输出与输入反相。零偏且处在恒流区时，还可由

$$
g_m=g_{m0}=\frac{2I_{DSS}}{\lvert V_{GS(\mathrm{off})}\rvert}
$$

直接求跨导。

输入端虽然 MOS 栅极本身近似开路，但图中的 $R_G$ 接在输入端到交流地之间，所以：

$$
R_i\approx R_G
$$

求输出电阻时，令输入信号为零并撤去 $R_L$，此时 $v_{gs}=0$、受控源电流为零：

$$
R_o\approx R_D
$$

若题目要求考虑沟道长度调制，则把增益括号内改为 $R_D\parallel R_L\parallel r_{ds}$，并取 $R_o\approx R_D\parallel r_{ds}$。

> [!warning] 这一页的两步检查
> 1. 直流先检查 $V_{DSQ}\ge -V_{GS(\mathrm{off})}$，确认能不能取 $I_{DQ}=I_{DSS}$。
> 2. 交流再写 $A_u=v_o/v_i$；求 $R_o$ 时必须撤去负载 $R_L$，不能把 $R_L$ 并进输出电阻。
