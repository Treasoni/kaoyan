...
![[考研专业课/模拟电子技术/附件/02-场效应管及其放大电路/手写-N沟道JFET自偏压共源-02-交流通路与动态指标-2026-08-06.jpg|520]]

从转移特性对 $v_{GS}$ 求导，可得 JFET 的互导：

$$
g_m=\left.\frac{\mathrm d i_D}{\mathrm d v_{GS}}\right\rvert_Q
=-\frac{2I_{DSS}}{V_P}\left(1-\frac{V_{GSQ}}{V_P}\right)
$$

由于 N 沟道 JFET 的 $V_P<0$，上式计算出的 $g_m$ 为正值。

令零偏互导 $g_{m0}=2I_{DSS}/\lvert V_P\rvert$，也常写成：

$$
g_m=g_{m0}\sqrt{\frac{I_{DQ}}{I_{DSS}}}
=\frac{2}{\lvert V_P\rvert}\sqrt{I_{DSS}I_{DQ}}
$$

其中 $I_{DQ}$ 是静态工作点漏极电流。分母是 $\lvert V_P\rvert$ 而不是 $\lvert V_P\rvert^2$，否则量纲不会是电导。做小信号题时，先由直流偏置求 $V_{GSQ}$、$I_{DQ}$，再代入 $g_m$，最后进入共源、共漏或共栅等效电路。

### 12.4 带 $R_S$、$C_S$ 的 JFET 自偏压共源电路

![[考研专业课/模拟电子技术/附件/02-场效应管及其放大电路/手写-N沟道JFET自偏压共源-01-直流通路与Q点-2026-08-06.jpg|520]]

![[考研专业课/模拟电子技术/附件/02-场效应管及其放大电路/图-N沟道JFET自偏压共源-直流交流对照.png|780]]

> [!note] 图的作用：把“同一只 $R_S$ 直流稳定、交流被 $C_S$ 旁路”的两种身份分开
> 左图只保留 $V_{DD}$、$R_D$、JFET、$R_S$、$R_G$，用于求 Q 点；右图把耦合/旁路电容视为短路、$V_{DD}$ 视为交流地，$R_D$ 与 $R_L$ 并联。考题必须先分清正在画直流图还是交流图。

#### 12.4.1 直流：$R_S$ 形成自偏压负反馈

直流稳态时 $C_i,C_o,C_S$ 全部开路，信号源与 $R_L$ 均不参与直流通路。又因为 $I_G\approx0$，栅极经 $R_G$ 接地，故：

$$
V_G\approx0,\qquad V_S\approx I_{DQ}R_S,\qquad V_{GSQ}=-I_{DQ}R_S
$$

把它代入肖克莱方程可得到一个关于 $I_{DQ}$ 的隐式方程：

$$
I_{DQ}=I_{DSS}\left(1+\frac{V_{GSQ}}{\lvert V_P\rvert}\right)^2
=I_{DSS}\left(1-\frac{I_{DQ}R_S}{\lvert V_P\rvert}\right)^2
$$
...
```text
I_D 增大 -> V_S=I_D R_S 增大 -> V_GS=−V_S 更负 -> I_D 被拉回
```

> [!tip] 静态检查
> 除了栅源 PN 结反偏外，还要确认 $V_P<V_{GSQ}\le0$ 且 $V_{DSQ}\ge V_{GSQ}-V_P$。前者保证未截止，后者保证 Q 点仍在恒流区。

#### 12.4.2 交流：$C_S$ 旁路 $R_S$，恢复较大增益

![[考研专业课/模拟电子技术/附件/02-场效应管及其放大电路/图4-8-8a-JFET共源极电路.jpg|520]]

![[考研专业课/模拟电子技术/附件/02-场效应管及其放大电路/图4-8-8b-JFET共源小信号等效.jpg|520]]

中频下 $C_S$ 近似短路，$R_S$ 被旁路，源极为交流地。若忽略 $r_{ds}$，并把负载接入后的漏极等效电阻记为 $R_D'=R_D\parallel R_L$：
...
若保留 $r_{ds}$，只需将括号内改为 $R_D\parallel R_L\parallel r_{ds}$。

输入电阻常由栅极偏置电阻决定。图中 JFET 栅极本身近似不取电流，但输入端仍经 $R_G$ 接交流地，因此：

$$
R_i\approx R_G
$$

求输出电阻时，令输入信号源为零、撤去 $R_L$。此时 $v_{gs}=0$，受控源电流为零，因此：

$$
R_o\approx R_D
$$

若题目没有旁路电容 $C_S$，才不能把源极当作交流地；这时应使用：

$$
A_v=-\frac{g_m(R_D\parallel R_L)}{1+g_mR_S}
$$

> [!warning] 本页最容易混用的两组公式
> - 有 $C_S$ 且中频可视为短路：$A_v\approx-g_m(R_D\parallel R_L)$。
> - 无 $C_S$（或低频时 $C_S$ 不足以旁路）：$A_v\approx-\dfrac{g_m(R_D\parallel R_L)}{1+g_mR_S}$。
>
> 直流计算永远把 $C_S$ 看成开路，不能因为它会提高交流增益而从 Q 点方程中删掉 $R_S$。
...
- [ ] 看到 $C_S$ 先问：本题求直流还是中频交流？直流开路保留 $R_S$；中频短路旁路 $R_S$。
