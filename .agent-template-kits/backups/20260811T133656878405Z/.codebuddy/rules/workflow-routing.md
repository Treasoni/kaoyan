...
```text
.codebuddy/workflows/{workflow-id}/workflow.md        # workflow definition
.codebuddy/workflows/{workflow-id}/state-template.md # state file template
...
- Phase state must be changed only through `.codebuddy/scripts/todo-state.sh`.
- Each workflow directory must have a `routing.yaml`; it is the source of truth for the generated table above.
- After creating, changing, renaming, or deleting a workflow, run `.codebuddy/scripts/sync-workflow-routing.sh`. Use `.codebuddy/scripts/sync-workflow-routing.sh --check` in pre-commit or CI.
