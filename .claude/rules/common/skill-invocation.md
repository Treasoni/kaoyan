...
| `kaoyan-math` | 考研数学入口路由器。用于数学二学习中的概念理解、笔记生成、推导补全、题目讲解、知识结构查询、进度/欠账检查。默认只加载一个最匹配子模块：conclusio… | 考研数学入口路由器 |
| `kaoyan-math-conclusions` | "将用户提供的考研数学证明、例题或解题结论，按统一结构追加到 Obsidian 的做题结论聚合笔记。 | 将用户提供的考研数学证明、例题或解题结论，按统一结构追加到 Obsidian 的做题结论聚合笔记。用户说“把这个做题结论放入”“整理进做题结论”“后续题目继续追加”或明确要求记录数学结论时使用；纯解释、章节总结、错题归档不使用。 |
