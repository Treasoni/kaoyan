...
处理用户已有章节笔记、截图、图片或要求更新“必背/速查/自测”类文件时，必须先按源笔记主线抽取，再压缩重排：

1. 先读取用户给出的章节笔记、当前笔记来源链接或图片/截图内容，不要先用 AI 另写一套表达。
2. 按“源笔记原句/公式/步骤 → 精简压缩 → 必要补充”的顺序处理。
3. 如果源笔记或原图已有主方法，必须把该方法放主位；其他方法只能作为“备用方法/补充理解”。
4. 必背/速查只保留定义、公式、性质、口诀、易错点、做题步骤、来源链接；长推导和完整例题留在源章节。
5. 结构识别型知识不能只剩公式：必须保留最小识别模板、做题启动步骤和必要图示/读图结论。
6. 源笔记缺少模板、启动步骤或图示时可以补充，但补充内容必须与源笔记主线一致。

### Markdown/LaTeX 写入验证清单 ⚠️

1. 写入含 LaTeX、反斜杠、代码块或表格的 Markdown 时，优先使用 Python raw string、占位符替换或 `chr(92)` 生成反斜杠。
2. 提取含中文括号、引号或特殊符号的标题/段落时，优先用 Python 读取文件并按标题边界定位，不用脆弱 shell 引号拼接。
3. 写入后扫描控制字符：
...
4. 回读目标段落，重点检查 `\\begin`、`\\boxed`、`\\theta`、`\\lambda`、`\\rvert`、`cases` 换行符和表格中的 LaTeX 管道符。
5. 若发现转义污染，先修复并重新扫描。
