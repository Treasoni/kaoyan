...
def normalized_text(text: str, dirs: dict[str, str]) -> str:
    replacements = {
        ".codebuddy/hooks": "{CODEX_HOOKS}",
        ".claude/hooks": "{CLAUDE_HOOKS}",
        ".codebuddy/settings.json": "{CODEX_SETTINGS}",
        ".claude/settings.json": "{CLAUDE_SETTINGS}",
        "Codex": "{CODEX}",
        "Claude Code": "{CLAUDE}",
        "codebuddy-hook": "{CODEX_HOOK}",
...
def transform_text(text: str, source: str, target: str, dirs: dict[str, str]) -> str:
    ...
    if source_kind == "codex" and target_kind == "claude":
        transformed = transformed.replace(".codebuddy/hooks", ".claude/hooks")
        transformed = transformed.replace(".codebuddy/settings.json", ".claude/settings.json")
        transformed = transformed.replace("CodeBuddy hook", "Claude Code hook")
        transformed = transformed.replace("codebuddy-hook", "claude-hook")
    elif source_kind == "claude" and target_kind == "codex":
        transformed = transformed.replace(".claude/hooks", ".codebuddy/hooks")
        transformed = transformed.replace(".claude/settings.json", ".codebuddy/settings.json")
        transformed = transformed.replace("Claude Code hook", "CodeBuddy hook")
        transformed = transformed.replace("claude-hook", "codebuddy-hook")
    return transformed
