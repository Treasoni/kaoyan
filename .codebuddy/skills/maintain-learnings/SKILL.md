...
```bash
python3 .codebuddy/skills/maintain-learnings/scripts/audit_learnings.py --root . --skills-dir .codebuddy/skills --rules-file CODEBUDDY.md --hooks-path .codebuddy/hooks
```
...
- 项目规则问题：`CODEBUDDY.md` / `CLAUDE.md` / `INSTRUCTIONS.md` / 项目自定义规则文件
- hook 问题：`.codebuddy/hooks/` / `.claude/hooks/` / 项目自定义 hook
- 工具脚本问题：对应 `scripts/`
...
```bash
python3 .codebuddy/skills/maintain-learnings/scripts/sync_platform_skills.py --root . --platform generic=.agent/skills --base-platform agents --compare-platform generic --skill <skill>
```

如果目标侧缺失，先 dry-run：

```bash
python3 .codebuddy/skills/maintain-learnings/scripts/sync_platform_skills.py --root . --from-platform agents --to-platform claude --skill <skill>
```

确认后应用：

```bash
python3 .codebuddy/skills/maintain-learnings/scripts/sync_platform_skills.py --root . --from-platform agents --to-platform claude --skill <skill> --apply
```
