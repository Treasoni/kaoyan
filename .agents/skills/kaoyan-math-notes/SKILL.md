...
1. **先读源笔记**：先读取用户给出的章节笔记、当前笔记来源链接或图片/截图内容，不要先用 AI 另写一套表达。
2. **抽取顺序**：按“源笔记原句/公式/步骤 → 精简压缩 → 必要补充”的顺序处理。
3. **保持主方法**：如果源笔记或原图已有主方法，必须把该方法放主位；其他可行方法只能写成“备用方法/补充理解”，不能覆盖原资料主线。
...
写入或替换含大量 LaTeX、反斜杠、代码块、表格的 Markdown 时，必须执行：

1. 优先使用 Python raw string、占位符替换或 `chr(92)` 生成反斜杠；避免普通三引号字符串、复杂 inline `printf` 或未加引号 heredoc。
2. 提取含中文括号、引号或特殊符号的标题/段落时，优先用 Python 读取文件并按标题边界定位，不用脆弱的 shell 引号拼接。
3. 写入后立刻扫描控制字符，只允许换行、回车和制表符：
...
5. 若发现转义污染，先修复并重新扫描；不得把受污染的笔记留给下一轮。
