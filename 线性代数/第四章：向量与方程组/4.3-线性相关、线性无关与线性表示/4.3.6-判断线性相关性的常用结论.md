...
> [!warning] ⚠️ 易混淆对比：结论4 vs 结论5
>
> | 对比项 | 结论4：部分与整体 | 结论5：延长与缩短 |
> |:-----:|:--------------:|:--------------:|
> | **关注点** | 向量**个数** | 向量**维数** |
> | **变化对象** | 从向量组中选取子集 | 增加/减少分量 |
> | **维度** | 不变 | 改变 |
> | **例子** | 5个向量中取3个 | 3维→5维 |
> | **口诀** | "部分推整体" | "无关可延长" |
...
> [!note] 📝 详细证明过程（矩阵秩法——"夹逼"推导）
> **思路**：利用增广矩阵秩的不等式进行两头堵。
>
> 记矩阵 $B = (\boldsymbol{\beta}_1, \boldsymbol{\beta}_2, \cdots, \boldsymbol{\beta}_s)$，增广矩阵 $\bar{B} = (\boldsymbol{\beta}_1, \boldsymbol{\beta}_2, \cdots, \boldsymbol{\beta}_s, \boldsymbol{\alpha})$。
>
> - 因为 $\boldsymbol{\beta}_1, \boldsymbol{\beta}_2, \cdots, \boldsymbol{\beta}_s$ **线性无关**，所以 $r(B) = s$
> - 因为 $\boldsymbol{\beta}_1, \boldsymbol{\beta}_2, \cdots, \boldsymbol{\beta}_s, \boldsymbol{\alpha}$ **线性相关**（共 $s+1$ 个向量），所以 $r(\bar{B}) < s+1$
> - 根据矩阵加列的秩不等式，有 $r(B) \leqslant r(\bar{B})$
>
> 综合上述条件，得到：
> $$s \leqslant r(\bar{B}) < s+1$$
>
> 由于矩阵的秩必须是整数，因此只能是 $r(\bar{B}) = s$。
>
> **收网结论**：因为 $r(B) = r(\bar{B}) = s$，根据非齐次线性方程组解的判定定理，方程组 $B\boldsymbol{x} = \boldsymbol{\alpha}$ 有解。又因为秩 $s$ 等于未知数的个数 $s$（即满秩无自由变量），所以方程组有且仅有**唯一解**。
>
> 故 $\boldsymbol{\alpha}$ 能由 $\boldsymbol{\beta}_1, \cdots, \boldsymbol{\beta}_s$ 唯一线性表示。$\blacksquare$

> [!tip] 💡 关键洞察
> 这个定理把**存在性**和**唯一性**统一在一个框架里：
> - **存在性**：由 $r(B) = r(\bar{B})$ 保证
> - **唯一性**：由 $r(B) = s$（满秩）保证
>
> 这就是线性代数中"秩"的强大之处——一个数字同时编码了两层信息。
