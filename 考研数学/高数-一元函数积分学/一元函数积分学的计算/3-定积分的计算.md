...
将原被积函数中的所有 $x$ 全部替换为 $t$ 的表达式。

$$f(x) \rightarrow f[\varphi(t)]$$

#### ② 积分变量要换（换微分）

对新变量 $t$ 求导，将原有的微分 $\mathrm{d}x$ 转化为新变量的微分 $\mathrm{d}t$。

$$\mathrm{d}x \rightarrow \varphi'(t)\mathrm{d}t$$

#### ③ 上下限要换

根据函数关系 $x = \varphi(t)$，由原积分的上下限计算出新积分的上下限：

- 当 $x = a$（原下限）时，求出对应的 $t = \alpha$（新下限）
- 当 $x = b$（原上限）时，求出对应的 $t = \beta$（新上限）

$$\int_{a}^{b} \rightarrow \int_{\alpha}^{\beta}$$

#### 💡 综合公式

将以上"三换"组合起来，就是定积分换元法的完整公式：
...
| 偶函数 | 奇函数 |
| :---: | :---: |
| ![](assets/3-定积分的计算/file-20260408093905460.png) | ![](assets/3-定积分的计算/file-20260408093900750.png) |
...
$$\int_0^\pi \sin^n x \mathrm{d}x = \begin{cases} 2 \cdot \dfrac{n-1}{n} \cdot \dfrac{n-3}{n-2} \cdots \dfrac{2}{3} \cdot 1, & n \text{ 为大于1的奇数} \\[10pt] 2 \cdot \dfrac{n-1}{n} \cdot \dfrac{n-3}{n-2} \cdots \dfrac{1}{2} \cdot \dfrac{\pi}{2}, & n \text{ 为正偶数} \end{cases}$$

**$\cos^n x$ 在 $[0, \pi]$ 上：**

$$\int_0^\pi \cos^n x \mathrm{d}x = \begin{cases} 0, & n \text{ 为正奇数} \\[10pt] 2 \cdot \dfrac{n-1}{n} \cdot \dfrac{n-3}{n-2} \cdots \dfrac{1}{2} \cdot \dfrac{\pi}{2}, & n \text{ 为正偶数} \end{cases}$$

#### 扩展形式（积分区间 $[0, 2\pi]$）

$$\int_0^{2\pi} \sin^n x \mathrm{d}x = \int_0^{2\pi} \cos^n x \mathrm{d}x = \begin{cases} 0, & n \text{ 为正奇数} \\[10pt] 4 \cdot \dfrac{n-1}{n} \cdot \dfrac{n-3}{n-2} \cdots \dfrac{1}{2} \cdot \dfrac{\pi}{2}, & n \text{ 为正偶数} \end{cases}$$

#### 公式汇总表

| 积分 | $n$ 为正奇数 | $n$ 为正偶数 |
|------|-------------|-------------|
| $\displaystyle\int_0^{\frac{\pi}{2}} \sin^n x \mathrm{d}x$ | $\dfrac{n-1}{n} \cdot \dfrac{n-3}{n-2} \cdots \dfrac{2}{3} \cdot 1$ | $\dfrac{n-1}{n} \cdot \dfrac{n-3}{n-2} \cdots \dfrac{1}{2} \cdot \dfrac{\pi}{2}$ |
| $\displaystyle\int_0^{\frac{\pi}{2}} \cos^n x \mathrm{d}x$ | $\dfrac{n-1}{n} \cdot \dfrac{n-3}{n-2} \cdots \dfrac{2}{3} \cdot 1$ | $\dfrac{n-1}{n} \cdot \dfrac{n-3}{n-2} \cdots \dfrac{1}{2} \cdot \dfrac{\pi}{2}$ |
| $\displaystyle\int_0^\pi \sin^n x \mathrm{d}x$ | $2 \cdot \dfrac{n-1}{n} \cdot \dfrac{n-3}{n-2} \cdots \dfrac{2}{3} \cdot 1$ | $2 \cdot \dfrac{n-1}{n} \cdot \dfrac{n-3}{n-2} \cdots \dfrac{1}{2} \cdot \dfrac{\pi}{2}$ |
...
| $\displaystyle\int_0^{2\pi} \cos^n x \mathrm{d}x$ | $0$ | $4 \cdot \dfrac{n-1}{n} \cdot \dfrac{n-3}{n-2} \cdots \dfrac{1}{2} \cdot \dfrac{\pi}{2}$ |

> [!tip] 记忆口诀
> - **奇数时**：连乘到 1（末尾是 1）
> - **偶数时**：连乘到 $\dfrac{1}{2}$ 后再乘 $\dfrac{\pi}{2}$
> - **区间扩展**：$[0, \pi]$ 乘 2，$[0, 2\pi]$ 乘 4（但 $\cos^n x$ 在 $[0, \pi]$ 和 $[0, 2\pi]$ 上奇数时为 0）
