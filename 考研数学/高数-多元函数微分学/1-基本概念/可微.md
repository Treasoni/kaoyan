...
> [!warning] 前置确认
> 可微必然要求 $f'_x(x_0,y_0)$ 和 $f'_y(x_0,y_0)$ 都存在。若有一个偏导数不存在，则函数在该点**不可微**；若两者都存在，再进入下面三步。
...
$$
\boxed{\lim_{\rho\to 0}
\frac{\Delta z-L}{\rho}=0\;?}
$$

| 结果 | 结论 |
|:----:|:----:|
| 极限 $=0$ | **可微**：全增量 = 线性增量 $+o(\rho)$ |
| 极限 $\neq 0$ 或不存在 | **不可微**：线性近似不满足定义 |

> [!tip] 核心逻辑
> 三步法检查的不是“全增量是否等于线性增量”，而是两者的误差相对于位移长度 $\rho$ 是否趋于 $0$：
> $$
> \Delta z=L+o(\rho)
> \Longleftrightarrow
> \frac{\Delta z-L}{\rho}\to 0.
> $$
> 其中 $\Delta z$ 是曲面的真实变化，$L=A\Delta x+B\Delta y$ 是切平面的线性预测。必须验证 $\Delta x,\Delta y$ **同时趋于 $0$** 的二元极限，不能只检验某一条坐标轴。
