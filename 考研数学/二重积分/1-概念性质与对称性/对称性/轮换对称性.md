...
> [!note] 解
> $\iint_D (2x^{2} + 3y^{2}) \mathrm{d}x \mathrm{d}y$ 与 $\iint_D (2y^{2} + 3x^{2}) \mathrm{d}y \mathrm{d}x$ 是两个不同的函数式（被积函数不同），但因为 $D$ 关于 $y = x$ 对称，将 $x$ 与 $y$ 对调后区域不变，**积分值相等**（几何上体积相同）。这正是下文要介绍的轮换对称性。
